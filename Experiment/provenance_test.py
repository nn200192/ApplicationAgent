...
def last_write_to_R(filename, length, plotname):
    ...
    f.write('pdf( \"plot' + plotname + '.pdf\", height=7, width=15) \n')
    f.write('myplots \n')
...
if __name__ == "__main__":
    ...
    for batch in batches:
        dates, timelines, df_list = process_file(batch)

        fields=["Dates", "Time", "Mem", "CPU", "Labels","Annotations", "Full_Annotations","DAG_if_root"]
        with open("plot" + timelines[0] + ".csv", 'a', encoding='UTF8') as csvfile:  
            csvwriter = csv.writer(csvfile) 
            csvwriter.writerow(fields)

        counter = 0
        for timeline, df in zip(timelines,df_list):
            ...
            if counter == 0:
                first_write_to_R("plot" + str(timelines[0]) + ".R")
            write_to_R(labels, graphs, "plot" + str(timelines[0]) + ".R", timeline, counter)
            write_to_csv(dates, "plot" + timelines[0] + ".csv", x, y, z, l, t, f_t, dag)
            counter = counter+1
        last_write_to_R("plot" + str(timelines[0]) + ".R", counter, timelines[0])
